...
async function run() {
    for (let i = 0; i < lines.length; i++) {
        ...
        // Match the local image require statement we already added and revert to wsrv.nl
        if (line.includes('image: ') || line.includes('require(')) {
            if (currentId && EXACT_IMAGES[currentId]) {
                const url = EXACT_IMAGES[currentId];
                // strip out 'https://' from the start for wsrv
                const cleanUrl = url.replace('https://', '');
                result.push(`    image: "https://wsrv.nl/?url=${cleanUrl}&w=400",`);
                replaced++;
            } else {
                result.push(line);
            }
    ...
    console.log(`Updated ${replaced} images using robust wsrv.nl endpoint to fix Native ORB.`);
}
